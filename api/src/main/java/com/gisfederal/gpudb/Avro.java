...
public final class Avro {
     ...
     * objects.
     */
    private static final class AvroDecodeProxy implements IndexedRecord {
        private final TypeObjectMap typeObjectMap;
        private final Schema schema;
        public Object object;

        public AvroDecodeProxy(TypeObjectMap typeObjectMap) {
     ...
     * objects.
     */
    private static final class AvroEncodeProxy implements IndexedRecord {
        private final TypeObjectMap typeObjectMap;
        private final Schema schema;
        public Object object;

        public AvroEncodeProxy(TypeObjectMap typeObjectMap) {
            this.typeObjectMap = typeObjectMap;
            this.schema = typeObjectMap.getSchema();
        }
    ...
    public static <T> T decode(Object typeDescriptor, ByteBuffer encodedObject) throws GPUdbException {
        if (typeDescriptor instanceof Class && IndexedRecord.class.isAssignableFrom((Class)typeDescriptor)) {
            ...
            TypeObjectMap typeObjectMap = (TypeObjectMap)typeDescriptor;
            T object = (T)typeObjectMap.newInstance();
            AvroDecodeProxy proxy = new AvroDecodeProxy(typeObjectMap);
            proxy.object = object;
    ...
    public static <T> List<T> decode(Object typeDescriptor, List<ByteBuffer> encodedObjects, int start, int count) throws GPUdbException {
        ...
        try {
            if (typeDescriptor instanceof Class && IndexedRecord.class.isAssignableFrom((Class)typeDescriptor)) {
                ...
                for (int i = start; i < start + count; i++) {
                    Object object = typeObjectMap.newInstance();
                    proxy.object = object;
                    decoder = factory.binaryDecoder(encodedObjects.get(i).array(), decoder);
     ...
     * @throws GPUdbException if an encoding error occurs
     */
    public static <T> ByteBuffer encode(TypeObjectMap typeObjectMap, T object) throws GPUdbException {
        AvroEncodeProxy proxy = new AvroEncodeProxy(typeObjectMap);
        proxy.object = object;
        return encode(proxy);
    }
     ...
     */
    private static <T> List<ByteBuffer> encodeInternal(TypeObjectMap typeObjectMap, List<T> objects, int start, int count) throws GPUdbException {
        if (start < 0) {
            throw new IndexOutOfBoundsException("Invalid start index specified.");
        }
     ...
     * @throws GPUdbException if an encoding error occurs
     */
    public static <T> List<ByteBuffer> encode(TypeObjectMap typeObjectMap, List<T> objects, int start, int count) throws GPUdbException {
     ...
     */
    private static <T> List<ByteBuffer> encodeInternal(final TypeObjectMap typeObjectMap, final List<T> objects, int start, int count, int threadCount, ExecutorService executor) throws GPUdbException {
        if (threadCount == 1 || count <= 1) {
            return encodeInternal(typeObjectMap, objects, start, count);
        }
     ...
     * @throws GPUdbException if an encoding error occurs
     */
    public static <T> List<ByteBuffer> encode(TypeObjectMap typeObjectMap, List<T> objects, int start, int count, int threadCount, ExecutorService executor) throws GPUdbException {
     ...
     * @throws GPUdbException if an encoding error occurs
     */
    public static <T> List<ByteBuffer> encode(TypeObjectMap typeObjectMap, List<T> objects) throws GPUdbException {
     ...
     * @throws GPUdbException if an encoding error occurs
     */
    public static <T> List<ByteBuffer> encode(TypeObjectMap typeObjectMap, List<T> objects, int threadCount, ExecutorService executor) throws GPUdbException {
